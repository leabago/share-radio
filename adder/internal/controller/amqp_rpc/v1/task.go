package v1

import (
	"context"
	"fmt"

	"github.com/goccy/go-json"
	"github.com/leabago/share-radio/adder/internal/controller/amqp_rpc/v1/request"
	"github.com/leabago/share-radio/adder/internal/controller/amqp_rpc/v1/response"
	"github.com/leabago/share-radio/adder/internal/entity"
	"github.com/leabago/share-radio/adder/pkg/rabbitmq/rmq_rpc/server"
	amqp "github.com/rabbitmq/amqp091-go"
)

func (r *V1) createTask() server.CallHandler {
	return func(ctx context.Context, d *amqp.Delivery) (any, error) {
		userID, data, err := extractUserID(d, r.j)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - createTask - auth: %w", err)
		}

		var req request.CreateTask

		err = json.Unmarshal(data, &req)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - createTask - json.Unmarshal: %w", err)
		}

		if err = r.v.Struct(req); err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - createTask - validation: %w", err)
		}

		task, err := r.tk.Create(ctx, userID, req.Title, req.Description)
		if err != nil {
			r.l.Error(err, "amqp_rpc - V1 - createTask")

			return nil, fmt.Errorf("amqp_rpc - V1 - createTask: %w", err)
		}

		return task, nil
	}
}

func (r *V1) getTask() server.CallHandler {
	return func(ctx context.Context, d *amqp.Delivery) (any, error) {
		userID, data, err := extractUserID(d, r.j)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - getTask - auth: %w", err)
		}

		var req request.GetTask

		err = json.Unmarshal(data, &req)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - getTask - json.Unmarshal: %w", err)
		}

		if err = r.v.Struct(req); err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - getTask - validation: %w", err)
		}

		task, err := r.tk.Get(ctx, userID, req.ID)
		if err != nil {
			r.l.Error(err, "amqp_rpc - V1 - getTask")

			return nil, fmt.Errorf("amqp_rpc - V1 - getTask: %w", err)
		}

		return task, nil
	}
}

func (r *V1) listTasks() server.CallHandler {
	return func(ctx context.Context, d *amqp.Delivery) (any, error) {
		userID, data, err := extractUserID(d, r.j)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - listTasks - auth: %w", err)
		}

		var req request.ListTasks

		err = json.Unmarshal(data, &req)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - listTasks - json.Unmarshal: %w", err)
		}

		if err = r.v.Struct(req); err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - listTasks - validation: %w", err)
		}

		var status *entity.TaskStatus

		if req.Status != "" {
			s := entity.TaskStatus(req.Status)
			status = &s
		}

		tasks, total, err := r.tk.List(ctx, userID, status, req.Limit, req.Offset)
		if err != nil {
			r.l.Error(err, "amqp_rpc - V1 - listTasks")

			return nil, fmt.Errorf("amqp_rpc - V1 - listTasks: %w", err)
		}

		return response.TaskList{Tasks: tasks, Total: total}, nil
	}
}

func (r *V1) updateTask() server.CallHandler {
	return func(ctx context.Context, d *amqp.Delivery) (any, error) {
		userID, data, err := extractUserID(d, r.j)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - updateTask - auth: %w", err)
		}

		var req request.UpdateTask

		err = json.Unmarshal(data, &req)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - updateTask - json.Unmarshal: %w", err)
		}

		if err = r.v.Struct(req); err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - updateTask - validation: %w", err)
		}

		task, err := r.tk.Update(ctx, userID, req.ID, req.Title, req.Description)
		if err != nil {
			r.l.Error(err, "amqp_rpc - V1 - updateTask")

			return nil, fmt.Errorf("amqp_rpc - V1 - updateTask: %w", err)
		}

		return task, nil
	}
}

func (r *V1) transitionTask() server.CallHandler {
	return func(ctx context.Context, d *amqp.Delivery) (any, error) {
		userID, data, err := extractUserID(d, r.j)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - transitionTask - auth: %w", err)
		}

		var req request.TransitionTask

		err = json.Unmarshal(data, &req)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - transitionTask - json.Unmarshal: %w", err)
		}

		if err = r.v.Struct(req); err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - transitionTask - validation: %w", err)
		}

		task, err := r.tk.Transition(ctx, userID, req.ID, entity.TaskStatus(req.Status))
		if err != nil {
			r.l.Error(err, "amqp_rpc - V1 - transitionTask")

			return nil, fmt.Errorf("amqp_rpc - V1 - transitionTask: %w", err)
		}

		return task, nil
	}
}

func (r *V1) deleteTask() server.CallHandler {
	return func(ctx context.Context, d *amqp.Delivery) (any, error) {
		userID, data, err := extractUserID(d, r.j)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - deleteTask - auth: %w", err)
		}

		var req request.DeleteTask

		err = json.Unmarshal(data, &req)
		if err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - deleteTask - json.Unmarshal: %w", err)
		}

		if err = r.v.Struct(req); err != nil {
			return nil, fmt.Errorf("amqp_rpc - V1 - deleteTask - validation: %w", err)
		}

		err = r.tk.Delete(ctx, userID, req.ID)
		if err != nil {
			r.l.Error(err, "amqp_rpc - V1 - deleteTask")

			return nil, fmt.Errorf("amqp_rpc - V1 - deleteTask: %w", err)
		}

		return response.DeleteStatus{Status: "deleted"}, nil
	}
}
