...
func (r *Repo) AllGenres(ctx context.Context) ([]entity.Genre, error) {

	selectGenresReq := selectGenres
	...
	genre, err := pgx.CollectOneRow(row, pgx.RowToStructByName[entity.Genre])
	if err != nil {
		return nil, err
	}
...
func (r *Repo) CreateStation(ctx context.Context, task *entity.Station) (string, error) {
	...
	// Gent genre
	genre, err := r.GetGenreByName(ctx, task.Genre)
	if err != nil {
		return "", err
	}
